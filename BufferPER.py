...
class ReplayBuffer:
    ...
    def __init__(self, config):
        ...
        self.experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "done", "deltaQ", "deltaQ_alpha"])
        self.sum_deltaQ = np.zeros(self.num_agents) # agents have their on deltaQs so there are no overlaps in priorities
    ...
    def add(self, states, actions, rewards, next_states, dones):
        ...
        for a in range(self.num_agents): # a as num_agents
            e = self.experience(states[a,:], actions[a,:], rewards[a], next_states[a,:], dones[a], deltaQ[a], deltaQ_alpha[a])
    ...
    def mem_print_summary(self):
        ...
        for a in range(self.num_agents):
            ...
            print('MinabsQ : {:11.9f} MaxabsQ : {:11.9f} and meanQ = {:11.9f} while mean(abs(Q)) = {:11.9f}'.format(min(abs(CurQ)),max(abs(CurQ)),np.mean(CurQ),np.mean(abs(CurQ))))
            print('With weighted sum from AllQ being : {} and stored one is : {}'.format(WeightedSumQ, self.sum_deltaQ[a]))
            print('With weighted sum from AllQA being : {} and stored one is : {}'.format(WeightedSumQA, self.sum_deltaQA[a]))
            # print(AllQ)
            # print(abs(AllQ)**self.alpha)
    
            print('MinabsP : {:11.9f} MaxabsP : {:11.9f} and meanP = {:11.9f}'.format(min(abs(CurP)),max(abs(CurP)),np.mean(CurP)))
